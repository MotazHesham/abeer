    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.qualifications.update", [$qualification->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.qualification.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $qualification->name) }}">
                @if($errors->has('name'))
